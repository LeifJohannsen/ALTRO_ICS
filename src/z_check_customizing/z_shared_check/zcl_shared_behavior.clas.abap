CLASS zcl_shared_behavior DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS changeParameters
        IMPORTING
            whereConditionOverlappCust     TYPE string
            whereConditionFindingInterval  TYPE string
            validFrom                      TYPE dats
            validto                        TYPE dats.
  ...
  PRIVATE SECTION.
    ...
    DATA: whereConditionFindingInterval TYPE string,
          whereConditionOverlappCust TYPE string,
          validFrom         TYPE dats,
          validto           TYPE dats,
          draftTableName    TYPE string,
    ...
    DATA: lastMinDateInFindings TYPE dats VALUE '18000101',
          lastMaxDateInFindings TYPE dats VALUE '99991231'.
...
CLASS ZCL_SHARED_BEHAVIOR IMPLEMENTATION.
  ...
  METHOD changeParameters.
    ...
    me->whereconditionfindinginterval = whereconditionfindinginterval.
    me->whereconditionoverlappcust = whereconditionoverlappcust.
  ...
  METHOD getOverlappingCustomizingCount.
        ...
        SELECT COUNT( * )
            FROM  (draftTableName)
            WHERE (me->whereConditionOverlappCust)
              AND
  ...
  METHOD getFindingInterval.
    ...
    DATA: lv_maxDate_tmp TYPE zics_protocols-created_at,
        ...
        SELECT MAX( created_at )
            FROM ZI_PROTOCOLS
            WHERE (me->whereconditionfindinginterval)
            INTO @lv_maxDate_tmp.
...
*              when ms-SUCCESS     then 'S' )
  ...
  METHOD GETMINANDMAXDATES.
    ...
    IF me->linesCount = 2.
        ...
        ENDIF.
        IF ValidTo < '99991231'.
            "Vorgängersatz (Über Action "Delimit")
            ...
            validToMax = '99991231'.
        ELSE.
            "Nachfolgersatz (Über Action "Delimit")
    ...
    ElSE.
        "Komplett neuer Satz (Über Action "Anlegen)
        validFromMin = cl_abap_context_info=>get_system_date( ).
        validFromMax = me->validto.
